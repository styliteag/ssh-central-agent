...
log_debug() {
    [ "${DEBUG:-0}" == "1" ] && echo "$(color_stderr setaf 5)$(color_stderr bold)DEBUG:$(color_stderr sgr0) $(color_stderr setaf 5)$*$(color_stderr sgr0)" >&2
}
...
cleanup() {
    SCA_EXITING=true
    CLEANUP_DONE=false
    [ -n "$SSH_PID" ] && [ "$CLEANUP_DONE" == "false" ] && log_info "Cleaning up background processes..." && CLEANUP_DONE=true
    [ -n "$SSH_PID" ] && kill -0 "$SSH_PID" 2>/dev/null && log_info "Stopping SSH agent forwarder (PID $SSH_PID)" && kill -TERM "$SSH_PID" 2>/dev/null || true
    [ -n "$SSH_PID" ] && log_info "Killing SSH processes with ragent" && pkill -f " ragent " 2>/dev/null || true
    [ -n "$MUX_PID" ] && [ "$CLEANUP_DONE" == "false" ] && log_info "Cleaning up background processes..." && CLEANUP_DONE=true
    [ -n "$MUX_PID" ] && kill -0 "$MUX_PID" 2>/dev/null && log_info "Stopping multiplexer (PID $MUX_PID)" && kill -TERM "$MUX_PID" 2>/dev/null || true
    [ -n "$MUX_PID" ] && log_info "Killing Python multiplexer (sshagentmux.py)" && pkill -f "sshagentmux.py" 2>/dev/null || true
    [ -n "$MUX_PID" ] && [ -n "$RUST_MUX_PID" ] && log_info "Killing ssh-agent-mux (PID $RUST_MUX_PID)" && pkill -x "ssh-agent-mux" 2>/dev/null || true
    [ -n "$TAIL_LOG_PID" ] && [ "$CLEANUP_DONE" == "false" ] && log_info "Cleaning up background processes..." && CLEANUP_DONE=true
    [ -n "$TAIL_LOG_PID" ] && kill -0 "$TAIL_LOG_PID" 2>/dev/null && log_info "Stopping log tail process (PID $TAIL_LOG_PID)" && kill -TERM "$TAIL_LOG_PID" 2>/dev/null || true
    [ -n "$RUST_MUX_LOG" ] && [ -f "$RUST_MUX_LOG" ] && [ "$CLEANUP_DONE" == "false" ] && log_info "Cleaning up background processes..." && CLEANUP_DONE=true
    [ -n "$RUST_MUX_LOG" ] && [ -f "$RUST_MUX_LOG" ] && log_info "Removing temp log file: $RUST_MUX_LOG" && rm -f "$RUST_MUX_LOG" 2>/dev/null || true
    [ -n "$SSH_PID" ] && [ "$CLEANUP_DONE" == "false" ] && log_info "Cleaning up background processes..." && CLEANUP_DONE=true
    [ -n "$SSH_PID" ] && log_info "Removing SSH agent socket: $SCA_SSH_AUTH_SOCK" && rm -f "$SCA_SSH_AUTH_SOCK" 2>/dev/null || true
    [ -n "$MUX_PID" ] && [ "$CLEANUP_DONE" == "false" ] && log_info "Cleaning up background processes..." && CLEANUP_DONE=true
    [ -n "$MUX_PID" ] && log_info "Removing mux socket: $MUX_SSH_AUTH_SOCK" && rm -f "$MUX_SSH_AUTH_SOCK" 2>/dev/null || true
    [ "$CLEANUP_DONE" == "true" ] && log_info "Cleanup complete"
}

kill_if_exists() {
    local pid=$1 name=$2
    [ -n "$pid" ] && kill -0 "$pid" 2>/dev/null && log_info "Stopping $name (PID $pid)" && kill -TERM "$pid" 2>/dev/null || true
}
...
find_identity_file() {
    ...
    for key_file in ~/.ssh/id_ed25519 ~/.ssh/id_rsa ~/.ssh/id_ecdsa ~/.ssh/id_dsa; do
        [ -f "$key_file" ] && [ -r "$key_file" ] && echo "$key_file" && return 0
    done
    return 1
}
...
build_ssh_cmd() {
    ...
    [ "$USE_IDENTITY_FILE" == "true" ] && [ -n "$IDENTITY_FILE" ] && cmd="$cmd -i \"$IDENTITY_FILE\""
    echo "$cmd"
}
...
determine_security_level() {
    ...
    ssh_exit_code=$?
    [ "$ssh_exit_code" -ne 0 ] && log_error "SSH connection failed with exit code $ssh_exit_code" && log_error "SSH output: $ssh_output" && return 1
    my_level=$(echo "$ssh_output" | grep "level-" | grep -o -E "level-([0-9]*)" | cut -d"-" -f2 | sort -u | tail -1)
    [ -z "$my_level" ] && log_error "Could not determine your security level. Check your SSH connection." && [ -n "$ssh_output" ] && log_error "SSH command output: $ssh_output" && return 1
    echo "$my_level"
    return 0
}

start_remote_agent() {
    local level=$1 ssh_cmd ssh_pid
    ssh_cmd=$(build_ssh_cmd)
    ...
    while [ $iterations -lt $SSH_RETRY_MAX ]; do
        ((iterations++))
        ! kill -0 "$ssh_pid" 2>/dev/null && log_error "SSH process (PID $ssh_pid) died unexpectedly" && return 1
        [ -S "$SCA_SSH_AUTH_SOCK" ] && check_agent_socket "$SCA_SSH_AUTH_SOCK" && echo "$ssh_pid" && return 0
    ...
    kill -0 "$ssh_pid" 2>/dev/null && log_error "SSH process (PID $ssh_pid) is still running but socket is not working" && kill -TERM "$ssh_pid" 2>/dev/null
    return 1
}

wait_for_socket() {
    local socket=$1 max_iterations=${2:-$SOCKET_WAIT_MAX} delay=${3:-$SOCKET_WAIT_DELAY} iterations=0
...
log_error() {
    echo "$(color setaf 1)$(color bold)ERROR:$(color sgr0) $(color setaf 1)$*$(color sgr0)" >&2
}

log_warn() {
    echo "$(color setaf 3)$(color bold)WARNING:$(color sgr0) $(color setaf 3)$*$(color sgr0)" >&2
}

log_info() {
    echo "$(color setaf 4)$(color bold)INFO:$(color sgr0) $(color setaf 4)$*$(color sgr0)" >&2
}

log_success() {
    echo "$(color setaf 2)$(color bold)✓$(color sgr0) $(color setaf 2)$*$(color sgr0)" >&2
}

log_debug() {
    if [ "${DEBUG:-0}" == "1" ]; then
        echo "$(color setaf 5)$(color bold)DEBUG:$(color sgr0) $(color setaf 5)$*$(color sgr0)" >&2
    fi
}

log_note() {
    echo "$(color setaf 6)$(color bold)NOTE:$(color sgr0) $(color setaf 6)$*$(color sgr0)" >&2
}
...
cleanup() {
    ...
    # Track if we actually do any cleanup
    CLEANUP_DONE=false
    ...
    # Clean up socket files if we started them (only if we actually started processes)
    if [ -n "$SSH_PID" ]; then
        if [ "$CLEANUP_DONE" == "false" ]; then
            log_info "Cleaning up background processes..."
            CLEANUP_DONE=true
    ...
    # Only remove mux socket if we started the mux ourselves (MUX_PID is set when we start it directly, empty for service)
    if [ -n "$MUX_PID" ]; then
        if [ "$CLEANUP_DONE" == "false" ]; then
            log_info "Cleaning up background processes..."
            CLEANUP_DONE=true
...
#==============================================================================
# VALIDATION: Check Local SSH Agent or Identity File
#==============================================================================
...
if [ "$MY_SOCKS_WORKING" == "false" ]; then
  ...
  if [ "$SKIP_MULTIPLEXER" != "true" ]; then
    ...
    if [ "$USE_RUST_MUX" == "true" ]; then
      #--------------------------------------------------------------------------
      # Rust Multiplexer (ssh-agent-mux) - Service Mode
      #--------------------------------------------------------------------------
...
EOF
      ...
      #--------------------------------------------------------------------------
      # Python Multiplexer (sshagentmux.py)
      #--------------------------------------------------------------------------
